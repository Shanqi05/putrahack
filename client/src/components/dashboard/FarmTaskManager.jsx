...
const FarmTaskManager = () => {
    ...
    const handleSaveTask = async (e) => {
        ...
        if (!newTask.title || !newTask.time) {
            alert("Please fill in both the task name and time!");
            return;
        }
    ...
    useEffect(() => {
        const checkTime = setInterval(() => {
            ...
            tasks
                .filter((t) => isTaskActiveOnDate(t, now))
                .forEach((task) => {
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl shadow-emerald-900/5 border border-emerald-50 h-full flex flex-col">
            ...
            {showAddForm && (
                <form
                    ...
                    className="mb-4 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 space-y-3"
                >
                    <input
                        type="text"
                        placeholder="Task name..."
                        className="w-full px-3 py-2 rounded-xl border border-emerald-200 focus:border-emerald-500 outline-none text-sm font-medium"
                        value={newTask.title}
                    ...
                    <div className="flex gap-2">
                        <input
                            type="time"
                            className="flex-1 px-3 py-2 rounded-xl border border-emerald-200 focus:border-emerald-500 outline-none text-sm"
                            value={newTask.time}
                            onChange={(e) => setNewTask({ ...newTask, time: e.target.value })}
                            required
                        />
                        <select
                            className="flex-1 px-3 py-2 rounded-xl border border-emerald-200 focus:border-emerald-500 outline-none text-sm bg-white"
                    ...
                    <button
                        type="submit"
                        className="w-full bg-emerald-500 text-white font-bold py-2 rounded-full hover:bg-emerald-600 text-sm mt-1"
                    >
                    ...
                    currentTasks
                        ...
                        .map((task) => {
                            ...
                            return (
                                <div
                                    key={task.id}
                                    className={`flex items-start gap-3 p-3 rounded-2xl border transition-all ${isCompleted ? "bg-gray-50 border-gray-100 opacity-60" : "bg-white border-gray-100 hover:border-emerald-200 hover:shadow-md hover:shadow-emerald-900/5"}`}
                                >
                                    <div
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p
                                            className={`font-bold text-sm truncate ${isCompleted ? "text-gray-500 line-through" : "text-emerald-950"}`}
                                        >
                                            {task.title}
                                        </p>
                                        <div className="flex items-center gap-3 mt-1.5">
                      <span
                          className={`flex items-center text-xs font-bold ${isCompleted ? "text-gray-400" : "text-emerald-600"}`}
                      >
